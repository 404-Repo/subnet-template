...
def _score_images(
    ...
    with torch.no_grad():
        ...
        for img in images:
            ...
            image_features = models.model.encode_image(
                models.preprocess(img).unsqueeze(0).to(device)
            )
...
def _render_images(mesh_bytes: bytes, views=4):
    buffer = io.BytesIO(mesh_bytes)
    fuze_trimesh = trimesh.load(buffer, file_type="ply")
...
def _normalize(tri_mesh: Any):
    ...
    try:
        ...
        logging.warning("First version of code failed, doing the second version")
