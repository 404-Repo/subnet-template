...
class Validator(BaseValidatorNeuron):
    ...
    async def forward(self):
        ...
        responses = await self.dendrite.forward(
            ...
            synapse=TextTo3D(dummy_input=self.step),
            deserialize=False,
        )
    ...
    def score_response(self, synapse: TextTo3D) -> float:
        if synapse.dummy_output is None:
            return 0.0
        return 1.0 if synapse.dummy_output == synapse.dummy_input * 2 else 0.0

    def score_responses(self, responses: List[TextTo3D]) -> torch.FloatTensor:
